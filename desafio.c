...
typedef enum { FORWARD, LEFT, RIGHT, PARA } State;
...
int main() {
  ...
  // stores device IDs for the Light Sensor
  WbDeviceTag ls0 = wb_robot_get_device("light sensor");
  ...
  while (wb_robot_step(time_step) != -1) {
    ...
    sensorluz = wb_light_sensor_get_value(ls0);
    printf("%.1lf\n",sensorluz);
    ...
    switch (state) {
      ...
      case FORWARD:
        if (wb_light_sensor_get_value(ls0) >= 900){
          speed[0] = wb_motor_get_velocity(left_wheel)*parando; // Gradually decreases velocity
          speed[1] = wb_motor_get_velocity(right_wheel)*parando; // Gradually decreases velocit
          state = PARA;
      ...
      case RIGHT:
        ...
        break;
        case PARA:
          if (wb_light_sensor_get_value(ls0) >= 900){ //Check if the light sensor's reading is above a threshold of 750W/m2
            speed[0] = wb_motor_get_velocity(left_wheel)*parando; // Gradually decreases velocity
            speed[1] = wb_motor_get_velocity(right_wheel)*parando; // Gradually decreases velocit
            state = PARA;
          }
